...
function FormEsameLab(props) {
  ...
  const aggiungi = () => {
    ...
    const postListRef = ref_database(db, `terapisti/${props.item}/pazienti/${props.idPaziente}/PDTA/esamiLaboratorio`); 
  ...
  const aggiungiFile = () => {
    const storageRef = ref_storage(storage, `/file/esamiLaboratorio/${file.name}`);
  ...
  useEffect(() => {
    if (fileUrls) {
      const postListRef = ref_database(db, `terapisti/${props.item}/pazienti/${props.idPaziente}/PDTA/file/esamiLaboratorio`);
      const newPostRef = push(postListRef);
